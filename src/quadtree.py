...
class QuadTree(Arbre):
    def __init__(self, region, points, capacite):
        ...
        if len(points)>self.capacite: #Si la taille de la liste des points est plus grande que la capacité, il faut diviser en 4.
            ...
            for e in range(4): #On répète la méthode 4 fois (pour 4 zones)
                nregion.append(region.quart(e)) #On utilise la méthode Quart pour définir le quart (NO>SO>SE>NE)
                pointse.append([])
    ...
    def contains(self, point):
        #Question 3
        test1 = self.etiquette().region.contient(point) #On vérifie que la région contient le point
        test2 = point in self.elements()                #On verifie aussi que point est bien dans la liste des points
    ...
    def elements(self):
        ...
        """
        res = set(self.etiquette().points) #On récupère les points de la node actuelle
        for e in self.enfants():        #Pour chaque node enfant
            res = res | e.elements()    #On utilise une concaténation avec le res, et les éléments de l'enfant.
        return res

    def add(self,point):
        if len(self.etiquette().points) == self.capacite:       #Si l'ajout du point entraine un dépassement de capacité
            self.etiquette().points.add(point)
            self.__init__(self.etiquette().region, self.etiquette().points, self.capacite) #On initialise une nouvelle node
        else:
            self.etiquette().points.add(point)
